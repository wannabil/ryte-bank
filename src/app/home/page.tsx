...
type FeedItemType = "netflix" | "japan" | "coffee" | "bill" | "salary" | "invest";

export default function HomePage() {
  const [balance, setBalance] = useState(24681.00);
  const [feedItems, setFeedItems] = useState<FeedItemType[]>(["netflix", "japan", "coffee", "bill", "salary", "invest"]);
  const [isLoading, setIsLoading] = useState(false);
  const [isBalanceVisible, setIsBalanceVisible] = useState(true);
  const [isExpanded, setIsExpanded] = useState(false);

  const displayedItems = isExpanded ? feedItems : feedItems.slice(0, 3);
  ...
  const handleCancelSubscription = () => {
    withLoading(() => {
      ...
      toast.success("Subscription cancelled. You saved RM14.99");
    });
  };
  ...
  const handleTransferToGoal = () => {
    withLoading(() => {
      ...
      toast.success("Transferred RM400 to Japan Trip Goal");
    });
  };
  ...
  const handlePayBill = () => {
    withLoading(() => {
      ...
      toast.success("Internet bill paid successfully");
    });
  };

  const handleCheckSalary = () => {
    withLoading(() => {
      handleDismiss("salary");
      toast.info("Salary breakdown sent to your email");
    });
  };

  const handleInvestAction = () => {
    withLoading(() => {
      handleDismiss("invest");
      setBalance(prev => prev - 500);
      toast.success("Invested $500 in tech ETF");
    });
  };
    ...
    <div className="min-h-screen bg-white text-gray-900 pb-28 relative overflow-x-hidden">
      ...
      <main className="px-6 space-y-8">
        <motion.div
          ...
          className="bg-[#0000e6] p-[1px] rounded-2xl shadow-lg shadow-blue-200"
        >
          <div className="bg-[#0000e6] backdrop-blur-md rounded-2xl p-4 flex items-center gap-3">
             <div className="p-2 bg-white/20 rounded-full text-white">
               <TrendingUp size={20} />
             </div>
             <p className="text-sm font-medium leading-snug text-white">
               You’re on track to save <span className="text-white font-bold">RM1,337</span> this month.
             </p>
          </div>
        </motion.div>
        ...
        <motion.div 
          ...
          className="text-center py-6 relative flex flex-col items-center justify-center"
        >
            ...
            <div className="relative z-10">
              <h1 className="text-5xl font-mono font-bold text-gray-900 tracking-tighter">
                  {isBalanceVisible ? (
              ...
              <button 
                ...
                className="mx-auto mt-3 p-2 text-gray-400 hover:text-[#0000e6] transition-colors rounded-full hover:bg-blue-50 active:scale-95"
        ...
        <div className="relative z-10">
            <div className="flex items-center justify-between mb-4">
                ...
                {feedItems.length > 3 && (
                  <button onClick={handleViewAll} className="text-xs text-[#0000e6] font-bold uppercase tracking-wider active:opacity-70">
            ...
            <div className="space-y-4">
              <AnimatePresence mode="popLayout">
                {displayedItems.includes("netflix") && (
                    <motion.div
                    layout
                    key="netflix"
                    initial={{ opacity: 0, y: 20 }}
                ...
                {displayedItems.includes("coffee") && (
                    <motion.div
                    layout
                    key="coffee"
                    initial={{ opacity: 0, y: 20 }}
                ...
                {/* New Card 1: Upcoming Bill */}
                {displayedItems.includes("bill") && (
                    <motion.div
                    layout
                    key="bill"
                    initial={{ opacity: 0, y: 20 }}
                ...
                {/* New Card 2: Salary Incoming */}
                {displayedItems.includes("salary") && (
                    <motion.div
                    layout
                    key="salary"
                    initial={{ opacity: 0, y: 20 }}
                ...
                {/* New Card 3: Investment Opportunity */}
                {displayedItems.includes("invest") && (
                    <motion.div
                    layout
                    key="invest"
                    initial={{ opacity: 0, y: 20 }}
